...
function setMemClass(classId, callback) {
  console.log('set');
  rmysql.query('SELECT c.id,c.gymid,c.service,c.price,c.active,c.daypass,c.spots,c.featured,c.duration,c.instructor,c.image,c.desc FROM classes c WHERE id = ' + classId, function(err, result, fields){
    if(err || result.length < 1) {
      ...
      rmysql.query('SELECT sc.id FROM scheduledClass sc WHERE sc.datetime > NOW() AND sc.classid = ' + classId + ' ORDER BY sc.datetime', function(err, scResult, fields){
        var scArray = [];
...
function setMemScheduledClass(sclassId,callback) {
  ...
  rmysql.query('SELECT sc.id,sc.service,sc.classid,sc.datetime,sc.active,sc.price,sc.gymid,sc.spots FROM scheduledClass sc WHERE sc.id = ' + sclassId, function(err, result, fields){
    if(err || result.length < 1) {
      callback('no result', null);
    } else {
      var scObj = {};
      ...
      rmysql.query('SELECT s.id,s.userid,s.checkin FROM schedule s WHERE s.sclassid = ' + sclassId, function(err, pResult, fields){
