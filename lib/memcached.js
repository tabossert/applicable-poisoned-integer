...
function setMemClass(classId, callback) {
  ...
  statement = [
      'SELECT c.id,c.gymid,c.name,c.price,c.active,c.daypass,c.spots,c.featured,c.duration,c.instructor,c.image,c.desc '
...
function setMemScheduledClass(sclassId,callback) {
  ...
  statement = [
      'SELECT sc.id,sc.name,sc.classid,sc.datetime,sc.active,sc.price,sc.gymid,sc.spots '
