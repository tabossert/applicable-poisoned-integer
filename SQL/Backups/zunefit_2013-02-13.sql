...
# Database: zunefit
# Generation Time: 2013-02-14 02:11:08 +0000
# ************************************************************
...
CREATE TABLE `classes` (
  ...
  `image` varchar(30) COLLATE utf8_bin NOT NULL DEFAULT 'misc.jpg',
  PRIMARY KEY (`id`),
  KEY `id_gymid` (`gymid`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
...
CREATE TABLE `classTimes` (
  ...
  `classid` int(10) NOT NULL,
  `gymid` int(10) NOT NULL,
  `weekday` varchar(12) COLLATE utf8_bin NOT NULL,
  `time` time NOT NULL,
  PRIMARY KEY (`id`),
...
IF ROW_COUNT() > 0 THEN
    ...
    ELSE
		...
		ELSE
			SET @sID = sID;
			SET @gymid = gymid;
			SET @aQuery = CONCAT('INSERT INTO eventAudit (gid,com,paid,type,sid) SELECT ? AS gymid,round(s.price * g.commission/100,2) AS com,s.price - round(s.price * g.commission/100,2) AS paid,1 AS type,? AS sid FROM gyms g INNER JOIN schedule s ON g.id = s.gymid WHERE s.id = ?');
			PREPARE pQuery FROM @aQuery;
			EXECUTE pQuery USING @gymid,@sID,@sID;
			IF ROW_COUNT() < 1 THEN
			...
			ELSE
				SET @sID = sID;
				SET @gymid = gymid;
				SET @aQuery = CONCAT('UPDATE gymBilling gb INNER JOIN schedule s ON gb.gid = s.gymid INNER JOIN gyms g ON g.id = gb.gid SET zcom = gb.zcom + round(s.price * g.commission/100,2), gb.balance = gb.balance + s.price - round(s.price * g.commission/100,2) WHERE gb.gid = ? AND s.id = ?');
				PREPARE pQuery FROM @aQuery;
				EXECUTE pQuery USING @gymid,@sID;
				IF ROW_COUNT() < 1 THEN
					SET transMess = CONCAT('"status": "failed", "message": "unable to add event4"');
					ROLLBACK;
				ELSE
...
IF ROW_COUNT() > 0 THEN
	SET @sid = sID;
	SET @aQuery = CONCAT('INSERT INTO eventAudit (gid,com,paid,type,sid) SELECT s.gymid AS gymid,round(s.price * g.commission/100,2) AS com,s.price - round(s.price * g.commission/100,2) AS paid,2 AS type,? AS sid FROM gyms g INNER JOIN schedule s ON g.id = s.gymid WHERE s.id = ?');
	PREPARE pQuery FROM @aQuery;
	EXECUTE pQuery USING @sID,@sID;
	IF ROW_COUNT() < 1 THEN
		SET transMess = "unable to delete event1";
		ROLLBACK;
	ELSE		
		SET @sID = sID;
		SET @aQuery = CONCAT('UPDATE gymBilling gb INNER JOIN schedule s ON gb.gid = s.gymid INNER JOIN gyms g ON g.id = gb.gid SET zcom = gb.zcom - round(s.price * g.commission/100,2), gb.balance = gb.balance - s.price - round(s.price * g.commission/100,2) WHERE gb.gid = s.gymid AND s.id = ?');
		PREPARE pQuery FROM @aQuery;
		EXECUTE pQuery USING @sID;
		IF ROW_COUNT() < 1 THEN
			SET transMess = "unable to delete event2";
			ROLLBACK;
		ELSE
			SET @sID = sID;
			...
			IF ROW_COUNT() > 0 THEN
				SET transMess = "success";
			ELSE
				SET transMess = "unable to delete event3";
				ROLLBACK;
			END IF;
		END IF;
	END IF;
...
/*!50003 SET SESSION SQL_MODE=""*/;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`localhost`*/ /*!50003 PROCEDURE `refillBalance`(IN ltype VARCHAR(15), IN token VARCHAR(100), IN cToken VARCHAR(50))
BEGIN
...
IF ROW_COUNT() > 0 THEN
	SET @cToken = cToken;
	SET @aQuery = CONCAT('INSERT INTO uBillingAudit (uid,action,amount,timestamp,cToken) SELECT u.id,2,b.refillamount,NOW(),? FROM users u INNER JOIN balance b ON u.id = b.userid WHERE ',ltype,'_token = "', token,'"');
	PREPARE pQuery FROM @aQuery;
	EXECUTE pQuery USING @cToken;
	IF ROW_COUNT() < 1 THEN
		ROLLBACK;
	END IF;
ELSE
