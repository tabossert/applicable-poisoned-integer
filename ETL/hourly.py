...
try:
    with zcon:
        ...
        for row in rows:
            ...
            crows = zcur.fetchall()
            for crow in crows:
                zcur.execute("INSERT INTO barbell.classhourly (gymid,classid,visits,reservations,duration,amount,datetime) SELECT ts.gymid,ts.classid,(SELECT count(id) FROM tmpchkin WHERE classid = ts.classid) AS visits,count(ts.classid) AS reservations,ts.duration AS duration,SUM(ts.price) AS amount,'" + startHour + "' AS datetime FROM tmpsch ts WHERE ts.gymid = " + str(crow[0]) + " GROUP BY classid")
                if (zcur.rowcount < 1):
                    zcur.execute("INSERT INTO barbell.classhourly (gymid,classid,visits,reservations,duration,amount,datetime) SELECT " + gymid + "," + str(crow[0]) + ",0,0,0,0,'" + startHour + "'")
            
            zcur.execute("INSERT INTO barbell.gymhourly (gymid,visits,reservations,duration,price,amount,datetime) SELECT ts.gymid,(SELECT count(id) FROM tmpchkin WHERE classid = ts.classid) AS visits,count(ts.classid) AS reservations,ts.duration AS duration,ts.price,SUM(ts.price) AS amount,'" + startHour + "' AS datetime FROM tmpsch ts WHERE ts.gymid = " + str(crow[0]))
            if (zcur.rowcount < 1):
                zcur.execute("INSERT INTO barbell.gymhourly (gymid,visits,reservations,duration,price,amount,datetime) SELECT " + gymid + ",0,0,0,0,0,'" + startHour + "'")               
